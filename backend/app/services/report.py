...
def build_report_pdf(case: CaseDetail) -> bytes:
    ...
    story = [
        ...
        Paragraph("Evidence Findings (Deterministic Forensic Checks)", h2),
        Paragraph(
            "SHA-256 hashing, metadata extraction, and perceptual fingerprint matching are "
            "deterministic computations, not model predictions.",
            small,
        ),
        ...
        Paragraph("AI / Synthetic Media Detection (Model-Based Inference)", h2),
        Paragraph(
            f"Classification: <b>{case.ai_detection.classification or 'N/A'}</b> "
            f"({case.ai_detection.score}% confidence)",
            body,
        ),
        Paragraph(
            f"Model: {case.ai_detection.model_name or 'none'} "
            f"({case.ai_detection.model_version or 'n/a'}) · "
            f"Analyzed: {case.ai_detection.analyzed_at.isoformat() if case.ai_detection.analyzed_at else 'N/A'} · "
            f"ML-based: {'Yes' if case.ai_detection.is_ml_based else 'No — fallback result'}",
            small,
        ),
        Paragraph(
            case.ai_detection.explanation or "No explanation available.",
            small,
        ),
        Paragraph(
            "This is a probabilistic model estimate, not conclusive proof of authenticity "
            "or manipulation, and is reported separately from the deterministic checks above.",
            small,
        ),

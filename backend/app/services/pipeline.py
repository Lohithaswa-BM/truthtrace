...
AI/synthetic-media detection (`ai_detection`) uses a real ONNX Runtime
inference pipeline — see app.services.ai_detector — with an honest
INCONCLUSIVE fallback when no model is configured. The manipulation-*type*
classifier (Face Swap / Copy-Move / etc.) below it remains a DETERMINISTIC,
CLEARLY LABELED heuristic (data_source="demo"), separate from AI detection.
Hashing, metadata, and pHash/Hamming similarity matching are genuinely real
(data_source "live"/"indexed").
"""
...
from app.services.ai_detector import run_ai_detection
...
def run_detector(sha256: str, kind: str) -> dict[str, Any]:
    """Heuristic seed generator for the manipulation-*type* classifier
    (Face Swap / Copy-Move / etc. — see classify_manipulation) and its
    supporting signals/temporal chart. Deterministic per-file, clearly
    tagged data_source='demo'. Out of scope for Phase 5.4: genuine
    AI/synthetic-media detection now lives in app.services.ai_detector and
    no longer uses this function's `ai_score` for its own classification —
    `ai_score` here only seeds the separate manipulation-type heuristic.
    """
...
def build_explainability(ai: dict, manip: dict, prov: dict, fp: dict) -> dict[str, Any]:
    reasons = []
    if ai.get("classification") == "INCONCLUSIVE" or not ai.get("is_ml_based", True):
        reasons.append("AI-generation model inference was inconclusive or unavailable for this evidence")
    elif ai["score"] >= 60:
...
def compute_forensic_score(ai: dict, manip: dict, orig: dict, fp: dict) -> int:
    # When the AI detector is inconclusive (no model available, or a
    # genuinely borderline result), its 0-confidence score must NOT be
    # treated as evidence of authenticity — that would silently bias every
    # "model unavailable" case toward "looks fine". Redistribute its weight
    # across the remaining signals instead.
    ai_is_conclusive = ai.get("is_ml_based", True) and ai.get("classification") != "INCONCLUSIVE"
    if ai_is_conclusive:
        ai_component = ai["score"] * 0.4
        manip_weight, orig_weight, fp_weight = 0.25, 0.2, 0.15
    else:
        ai_component = 0.0
        # redistribute the 0.4 proportionally across the other three
        manip_weight, orig_weight, fp_weight = 0.25 / 0.6, 0.2 / 0.6, 0.15 / 0.6

    score = (
        ai_component + manip["score"] * manip_weight
        + orig["score"] * orig_weight + fp["similarity"] * fp_weight
    )
...
def analyze_file(db: Session, path: Path, filename: str) -> dict[str, Any]:
    ...
    ai_frame_path: Path | None = None  # representative frame for AI-detection inference
    _video_temp_dir = None
    ...
    if kind == "image":
        ...
        if ffmpeg_available():
            ...
            if frame_paths:
                ai_frame_path = frame_paths[0]  # kept alive until after AI detection runs below
                _video_temp_dir = frame_paths[0].parent
    ...
    # Phase 5.4: genuine ONNX-based AI/synthetic-media detection, isolated
    # from the heuristic manipulation-type classifier below. Falls back to
    # an honest INCONCLUSIVE result if no model is configured/available —
    # never fabricates a score.
    ai_result = run_ai_detection(ai_frame_path, kind)
    if _video_temp_dir is not None:
        import shutil as _shutil
        _shutil.rmtree(_video_temp_dir, ignore_errors=True)

    ai_detection = {
        "label": ai_result.label,
        "score": ai_result.score,
        "severity": ai_result.severity,
        "data_source": "live" if ai_result.is_ml_based else "demo",
        "classification": ai_result.classification,
        "model_name": ai_result.model_name,
        "model_version": ai_result.model_version,
        "analyzed_at": ai_result.analyzed_at.isoformat(),
        "explanation": ai_result.explanation,
        "is_ml_based": ai_result.is_ml_based,
    }
    ...
    explainability = build_explainability(ai_detection, manipulation, provenance, fingerprint)
    tracing = build_tracing(matches)
    forensic_score = compute_forensic_score(ai_detection, manipulation, originality, fingerprint)
    confidence_label = (
